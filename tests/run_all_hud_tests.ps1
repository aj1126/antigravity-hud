...
Write-Host "[1/4] Running Node.js Engine & CLI Matrix Tests..." -ForegroundColor Yellow
$res1 = node (Join-Path $testDir "hud_engine.test.js")
Write-Host $res1
if ($LASTEXITCODE -ne 0) { $allPassed = $false }

# 2. WinForms Headless Pester Tests
Write-Host "[2/4] Running WinForms Headless GUI Pester Tests..." -ForegroundColor Yellow
...
Write-Host "[3/4] Running Web GUI Template & Schema Tests..." -ForegroundColor Yellow
$res3 = node (Join-Path $testDir "hud_web_gui.test.js")
Write-Host $res3
if ($LASTEXITCODE -ne 0) { $allPassed = $false }

# 4. Settings Integration Tests
Write-Host "[4/4] Running Settings Integration Tests..." -ForegroundColor Yellow
